...
async function saveCloud() {
  ...
  try {
    const res = await fetchTimeout(DOC, { method: "PUT", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ v: 2, settings: state.settings, items: state.items, txns: state.txns }) }, 8000);
...
function stockAge(item) { const start = item.dateOfManufacture || item.createdOn; return start ? daysBetween(start, today()) : ""; }
...
async function startCam() {
  ...
  if (!window.Html5Qrcode) { document.getElementById("cam-hint").textContent = "Camera tool did not load. Use Add material or Upload Excel."; return; }
...
function onScan(text) {
  const c = String(text).trim(); const t = Date.now();
...
function openForm(prefill) {
  ...
  ["f-id","f-materialNo","f-mCode","f-description","f-bay","f-location","f-locationDesc","f-batchNo","f-vendorBatch","f-poNumber","f-poItem","f-salesName","f-salesDept","f-packaging","f-profitCenter","f-dom","f-exp"].forEach(function (id) { const el = document.getElementById(id); if (el) el.value = ""; });
...
function editForm(id) {
  const i = state.items.find(function (x) { return x.id === id; }); if (!i) return;
  document.getElementById("modal").classList.add("open");
...
function saveForm() {
  const materialNo = val("f-materialNo"); const description = val("f-description"); const batchNo = val("f-batchNo"); const bay = val("f-bay"); const qty = Number(document.getElementById("f-qty").value);
  if (!materialNo || !description || !batchNo || !bay || !(qty >= 0)) { toast("Need Material No, Description, Batch No, Bay, and Quantity.", true); return; }
  const rec = { id: val("f-id") || uid(), materialNo: materialNo, mCode: val("f-mCode"), description: description, bay: bay, location: val("f-location"), locationDesc: val("f-locationDesc"), batchNo: batchNo, vendorBatch: val("f-vendorBatch"), qty: qty, unit: document.getElementById("f-unit").value, createdOn: val("f-createdOn") || today(), dateOfManufacture: val("f-dom"), expirationDate: val("f-exp"), poNumber: val("f-poNumber"), poItem: val("f-poItem"), salesName: val("f-salesName"), salesDept: val("f-salesDept"), packaging: val("f-packaging"), profitCenter: val("f-profitCenter") };
...
function filteredItems() {
  ...
  return state.items.filter(function (i) {
    const hay = [i.materialNo, i.mCode, i.description, i.bay, i.location, i.batchNo, i.vendorBatch, i.poNumber].join(" ").toLowerCase();
...
function drawItems() {
  ...
  if (!rows.length) {
    const empty = "<div class='card'>No materials yet. Upload Excel/CSV or tap + Add material.</div>";
    if (el) el.innerHTML = empty; if (sheet) sheet.innerHTML = empty; return;
  }
  if (sheet) {
    const cols = ["Bay","Location","Material No","M code","English Description","Batch No","Vendor Batch","Quantity","Unit","Expiration Date","PO Number","Remaining days","Packaging",""];
...
function printLabel(id) {
  ...
  if (!i || !window.QRCode) return toast("Could not make QR yet. Wait a second and try again.", true);
  QRCode.toDataURL(i.id, { width: 280, margin: 1 }, function (err, url) {
    ...
    w.document.write("<html><body style='font-family:Arial;padding:16px'><div style='border:3px solid #0b1f3a;padding:16px;width:380px'><div style='background:#f5c518;font-weight:800;display:inline-block;padding:5px 8px'>FENCHEM PALLET</div><h1>" + escapeHtml(i.description) + "</h1><div>Material: <b>" + escapeHtml(i.materialNo) + "</b></div><div>Batch: <b>" + escapeHtml(i.batchNo) + "</b></div><div>Qty: <b>" + i.qty + " " + escapeHtml(i.unit) + "</b></div><div>Bay: <b>" + escapeHtml(i.bay || "not set") + "</b></div><img src='" + url + "' width='220' height='220' style='display:block;margin:14px auto'><div style='text-align:center'>" + escapeHtml(i.id) + "</div></div><script>setTimeout(function(){window.print()},200)<" + "/script></body></html>");
    w.document.close();
  });
}
function drawReport() {
  ...
  document.getElementById("stats").innerHTML = "<div class='stat'><span class='muted'>Pallets / lots</span><b>" + state.items.length + "</b></div><div class='stat'><span class='muted'>Total qty</span><b>" + roundQty(units) + "</b></div><div class='stat'><span class='muted'>Expiring in 30d</span><b>" + exp.length + "</b></div>";
...
function mapRow(headers, values) {
  function get() {
    ...
    for (let n = 0; n < names.length; n++) {
      ...
      if (i < 0) { for (let h = 0; h < headers.length; h++) { if (headers[h].indexOf(names[n]) >= 0 && headers[h] !== "location description") { i = h; break; } } }
  ...
  if (name.indexOf(".xlsx") >= 0 || name.indexOf(".xls") >= 0) {
    ...
    reader.onload = function () {
      try {
        ...
        importTable(XLSX.utils.sheet_to_json(wb.Sheets[wb.SheetNames[0]], { header: 1, raw: false, defval: "" }));
...
function boot() { try { loadCloud(); } catch (e) { setSync(false, "Ready"); } try { show("scan"); } catch (e) {} setInterval(loadCloud, 30000); }
