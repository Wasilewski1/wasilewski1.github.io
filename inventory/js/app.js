...
function findItem(code) {
  ...
  if (!c) return null;
  return (
    state.items.find((i) => String(i.id).toLowerCase() === c) ||
    state.items.find((i) => String(i.materialNo).toLowerCase() === c && String(i.batchNo).toLowerCase() === c) ||
...
function confirmMove(id) {
  ...
  state.txns.unshift({
    ...
    qty, unit: item.unit, result: item.qty,
    who: state.settings.operator || "Someone", note,
  });
  saveCloud(); paint();
...
function saveForm() {
  ...
  if (wantPrint) setTimeout(() => printLabel(rec.id), 250);
}
...
function drawItems() {
  ...
  if (!rows.length) {
    const empty = "<div class='card'>No materials yet. Tap <b>+ Add material</b>, fill the warehouse fields, then print the pallet QR and stick it on the pallet.</div>";
  ...
  if (sheet) {
    ...
    sheet.innerHTML = "<table><thead><tr>" + cols.map((c) => "<th>" + c + "</th>").join("") + "</tr></thead><tbody>" +
      rows.map((i) => {
        ...
        return "<tr>" + cells.map((c) => "<td>" + escapeHtml(c == null ? "" : c) + "</td>").join("") +
          "<td><button onclick='printLabel(\"" + i.id + "\")'>QR</button> " +
          "<button class='ghost' onclick='editForm(\"" + i.id + "\")'>Edit</button></td></tr>";
      }).join("") + "</tbody></table>";
  }
  if (!el) return;
  el.innerHTML = rows.map((i) => {
    ...
    return "<div class='item'><div class='row' style='align-items:flex-start'><div>" +
      ...
      "<div class='actions'>" +
      "<button onclick='printLabel(\"" + i.id + "\")'>Print pallet QR</button>" +
      "<button class='ghost' onclick='editForm(\"" + i.id + "\")'>Edit</button>" +
      "<button class='ghost' onclick=\"quick('" + i.id + "','in')\">In</button>" +
      "<button class='ghost' onclick=\"quick('" + i.id + "','out')\">Out</button>" +
      "</div></div>";
  }).join("");
}
...
function quick(id, type) {
  ...
  state.txns.unshift({
    at: new Date().toISOString(), type, itemId: item.id, name: item.description,
    materialNo: item.materialNo, batchNo: item.batchNo, qty: n, unit: item.unit,
    result: item.qty, who: state.settings.operator || "Someone", note: "typed",
  });
...
function printLabel(id) {
  ...
  QRCode.toDataURL(i.id, { width: 280, margin: 1 }, function (err, url) {
    ...
    w.document.write("<!doctype html><title>Pallet label</title><style>" +
      "body{font-family:Arial,sans-serif;padding:16px;color:#122033;background:#fff}" +
      ".box{border:3px solid #0b1f3a;padding:16px;width:380px}" +
      ".h{background:#f5c518;color:#0b1f3a;font-weight:800;letter-spacing:.1em;font-size:12px;padding:5px 8px;display:inline-block}" +
      "h1{font-size:18px;margin:10px 0 8px} .m{font-size:13px;line-height:1.45} b{color:#0b1f3a}" +
      "img{display:block;margin:14px auto 6px;width:220px;height:220px}" +
      ".id{text-align:center;font-family:Consolas,monospace;font-size:12px;letter-spacing:.04em}" +
      "@media print{body{padding:0}}" +
      "</style><div class='box'><div class='h'>FENCHEM PALLET</div>" +
      "<h1>" + escapeHtml(i.description) + "</h1>" +
      "<div class='m'>Material No: <b>" + escapeHtml(i.materialNo) + "</b></div>" +
      (i.mCode ? "<div class='m'>M code: <b>" + escapeHtml(i.mCode) + "</b></div>" : "") +
      "<div class='m'>Batch: <b>" + escapeHtml(i.batchNo) + "</b>" +
      ...
      "<img src='" + url + "' alt='QR' /><div class='id'>" + escapeHtml(i.id) + "</div></div>" +
...
function drawReport() {
  const exp = state.items.filter((i) => {
    const r = remainingDays(i);
    return r !== "" && r <= 30;
  ...
  document.getElementById("stats").innerHTML =
    ...
    "<div class='stat'><span class='muted'>Expiring ≤30d</span><b>" + exp.length + "</b></div>";
  document.getElementById("r-exp").innerHTML = exp.length
    ? "<table><thead><tr><th>Material</th><th>Batch</th><th>Days</th><th>Qty</th></tr></thead><tbody>" +
      exp.map((i) => "<tr><td>" + escapeHtml(i.description) + "<div class='muted'>" + escapeHtml(i.materialNo) +
      "</div></td><td>" + escapeHtml(i.batchNo) + "</td><td>" + remainingDays(i) + "</td><td>" + i.qty + " " +
      escapeHtml(i.unit) + "</td></tr>").join("") + "</tbody></table>"
  ...
  document.getElementById("r-hist").innerHTML = state.txns.length
    ? "<table><thead><tr><th>When</th><th></th><th>Material</th><th>Qty</th></tr></thead><tbody>" +
      state.txns.slice(0, 25).map((t) => "<tr><td>" + new Date(t.at).toLocaleString() + "</td><td>" +
      t.type.toUpperCase() + "</td><td>" + escapeHtml(t.name) + "</td><td>" + t.qty + " " +
      escapeHtml(t.unit || "") + "</td></tr>").join("") + "</tbody></table>"
